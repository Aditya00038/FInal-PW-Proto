...
export default function NormalGoalDetailsClient({ goal: initialGoal }: NormalGoalDetailsProps) {
  ...
  const refreshGoal = useCallback(() => {
    const updated = getNormalGoalById(goal.id);
  ...
  const handleDeposit = () => {
    ...
    const result = depositToNormalGoal(goal.id, amt, depositNote || undefined);
    if (result) {
      toast({ title: "Deposit Successful!", description: `₹${amt.toLocaleString("en-IN")} added to "${goal.name}".` });
      setDepositAmount("");
      setDepositNote("");
  ...
  const handleWithdraw = () => {
    ...
    const result = withdrawFromNormalGoal(goal.id, amt, withdrawNote || undefined);
    if (result) {
  ...
  const handleSaveFinancials = () => {
    updateNormalGoalFinancials(goal.id, {
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
            ...
            <Dialog open={isDepositOpen} onOpenChange={setIsDepositOpen}>
              <DialogTrigger asChild>
                <Button className="w-full" size="lg">
                  <PiggyBank className="mr-2 h-5 w-5" /> Deposit Money
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
          ...
          {/* Motivational CTA to Smart Contract */}
      ...
      {/* ── AI Goal Advisor & Prediction Engine ──────────── */}
